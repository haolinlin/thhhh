...
QWidgetDraw::QWidgetDraw(QWidget *parent) : QWidget(parent)
{
   ...
   this->map = new QPixmap(this->width(),this->height());
    ...
    //connect(this, &QWidgetDraw::sinDataSignal, this, &QWidgetDraw::refreshFromData);
    timer->start(10);//单位是ms
}

/*void QWidgetDraw ::testSend()
{
    int sindata=0;

    int height = this->height();
    sindata = height-sinTable[index]*height/512;

    index_sin +=1;
    if(index_sin >=sizeof(ecgWave)/sizeof(int))
        index_sin=0;

    emit rxDataSignal(sindata);
}*/
